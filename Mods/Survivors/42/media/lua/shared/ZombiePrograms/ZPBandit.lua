...
ZombiePrograms.Bandit.Main = function(bandit)
    ...
    local healthMin = 0.7
    local walkType = "Run"
    local peacefulMode = Bandit.IsPeacefulMode and Bandit.IsPeacefulMode()
    ...
    if not peacefulMode and ((isElectrician and SandboxVars.Bandits.General_GeneratorCutoff) or (isMechanic and SandboxVars.Bandits.General_SabotageVehicles)) then 
        for z=0, 1 do
            for y=-10, 10 do
                for x=-10, 10 do
    ...
    if not peacefulMode and SandboxVars.Bandits.General_Theft then
    ...
    if not peacefulMode and SandboxVars.Bandits.General_SabotageCrops then
