...
-- Bandit hostility
function Bandit.AreHostileSurvivorsEnabled()
    return SandboxVars
        and SandboxVars.Bandits
        and SandboxVars.Bandits.General_EnableHostileSurvivors == true
end

function Bandit.IsPeacefulMode()
    return not Bandit.AreHostileSurvivorsEnabled()
end

function Bandit.SetHostile(zombie, hostile)
    local brain = BanditBrain.Get(zombie)
    if brain then
        if Bandit.IsPeacefulMode() then hostile = false end
...
function Bandit.SetHostileP(zombie, hostileP)
    local brain = BanditBrain.Get(zombie)
    if brain then
        if Bandit.IsPeacefulMode() then hostileP = false end
...
function Bandit.IsHostile(zombie)
    if Bandit.IsPeacefulMode() then return false end
    local brain = BanditBrain.Get(zombie)
    if brain then
