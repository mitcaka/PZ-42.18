...
local function ManageCombat(bandit)
    ...
    local isDirectCompanion = SurvivorCompanions.IsDirectCompanion(brain)
    if isDirectCompanion and brain.companionDowned then
        return {}
    end
    if isDirectCompanion and brain.companionCombatMode == "passive" then
        if bandit:getHealth() < 0.4 and not BanditBrain.HasTaskType(brain, "Bandage") then
    ...
    if isDirectCompanion and brain.companionLoadoutMode == "melee" then
        isOutOfAmmo = true
    end
    ...
    local bestDist = isDirectCompanion and brain.companionCombatMode ~= "aggressive" and 18 or 40
                ...
                --- if true then
                    ...
                    if pesq and pesq:getLightLevel(0) > 0.20 and not bandit:getSquare():isSomethingTo(pesq) then
                        ...
                        if dist < bestDist then
                            ...
                            elseif not isOutOfAmmo and not (isDirectCompanion and brain.companionLoadoutMode == "balanced" and dist <= meleeDist) then
                                if weapons.primary.name and weapons.primary.bulletsLeft > 0 then
                                    if not maxRangeRifle then
                                        local item = BanditCompatibility.InstanceItem(weapons.primary.name)
                                        item = BanditUtils.ModifyWeapon(item, brain)
    ...
    elseif reload then
        if isDirectCompanion and brain.companionLoadoutMode == "melee" then return tasks end
        if not BanditBrain.HasActionTask(brain) then
            for _, slot in pairs({"primary", "secondary"}) do
...
local biteTab = {}
local CUSTOM_SURVIVOR_ACQUIRE_RANGE = 6
...
local function UpdateZombies(zombie)
        ...
        if tick == 14 then
            ...
            if dist < 0.8 then 
                ...
                if not bandit:isOnKillDone() then
                    ...
                    local downed = bandit:getHealth() <= 1.19 and SurvivorCompanions.SetDowned(bandit, "zombie bite")
                    if not downed then
                        bandit:Hit(teeth, zombie, 1.01, false, 1, false)
                        Bandit.UpdateInfection(bandit, 0.001)
                        if bandit:getHealth() > 0 and bandit:getHealth() <= 0.18 then
                            downed = SurvivorCompanions.SetDowned(bandit, "zombie bite")
                        end
                    end
    ...
    if currentTarget and currentTarget:getVariableBoolean("Bandit") then
        bandit = currentTarget
        enemy = {
            id = BanditUtils.GetCharacterID(bandit),
            x = bandit:getX(),
            y = bandit:getY(),
            z = bandit:getZ(),
            dist = BanditUtils.DistTo(zx, zy, bandit:getX(), bandit:getY()),
        }
    else
        -- Preserve vanilla player targeting and sound-investigation paths. Custom
        -- acquisition is only for an otherwise idle zombie with a nearby survivor.
        if currentTarget then return end

        enemy = BanditUtils.GetClosestBanditLocation(zombie)
        if enemy.dist >= CUSTOM_SURVIVOR_ACQUIRE_RANGE then return end

        bandit = BanditZombie.Cache[enemy.id]
        if not bandit or not zombie:CanSee(bandit) then return end

        local closestPlayer = BanditUtils.GetClosestPlayerLocation(zombie, {hearDist=0, levelDiff=1})
        if closestPlayer.x and closestPlayer.dist <= enemy.dist then return end
    ...
    -- Once vanilla or the close-range bridge selected a survivor, retain the
    -- custom bite simulation without replacing long-range investigate movement.
    if bandit and enemy.dist < 30 then
        local bx, by, bz = bandit:getX(), bandit:getY(), bandit:getZ()
...
local function OnBanditUpdate(zombie)
    ...
    if SurvivorCompanions.MaintainDowned(bandit, brain) then return end
    SurvivorCompanions.UpdateStuck(bandit, brain)
...
local function OnHitZombie(zombie, attacker, bodyPartType, handWeapon)
    ...
    if bandit:getHealth() > 0 and bandit:getHealth() <= 0.18
            and SurvivorCompanions.SetDowned(bandit, "critical wounds") then
        return
    end
    ...
    if handWeapon:isRanged() and instanceof(attacker, "IsoPlayer") then
        ...
        for k, tab in pairs(bodyPartTypes) do
            if BodyPartType[k] == bodyPartType then
                ...
                if rnd > def then
                    if tab.insta then
                        if SurvivorCompanions.SetDowned(bandit, "gunshot wound") then return end
                        bandit:Kill(nil)
                        return
                    end
                    if tab.serious then
