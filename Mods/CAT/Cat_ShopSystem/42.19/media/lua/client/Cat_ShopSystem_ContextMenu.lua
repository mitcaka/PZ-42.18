...
local function onFillWorldObjectContextMenu(playerNum, context, worldObjects, test)
    ...
    -- Only show when the player directly right-clicked on the register
    -- White register sprites = seller, black register sprites = buyer
    if not shopObj and not buyerObj and cashObj and cashObjDirect and isPlayerAdmin(player) then
...
-- ---------------------------------------------------------------------------
-- Left-click to open shop UI (mirrors context-menu routing)
-- ---------------------------------------------------------------------------
local function onObjectLeftMouseButtonUp(object, x, y)
    if not object then return end

    local player = getSpecificPlayer(0)
    if not player or not player:isAlive() or player:isAiming() then return end

    local isPaused = UIManager.getSpeedControls() and UIManager.getSpeedControls():getCurrentGameSpeed() == 0
    if isPaused then return end

    -- Don't intercept while placing an object or using a tool
    local cell = getCell()
    if cell and cell:getDrag(0) then return end

    -- Only handle initialized shop/buyer registers
    local isShop = isShopRegister(object)
    local isBuyer = isBuyerRegister(object)
    if not isShop and not isBuyer then return end

    -- Distance check
    local sq = object:getSquare()
    local psq = player:getCurrentSquare()
    if not sq or not psq then return end
    if psq:getZ() ~= sq:getZ() then return end
    if psq:DistTo(sq) > getShopRange() then return end

    -- Suppress the container loot panel that ISObjectClickHandler may have opened
    local lootPanel = getPlayerLoot(0)
    if lootPanel then
    ...
    require "Cat_ShopSystem_00_UI"
    Cat_ShopSystem.OpenShopUI(cx, cy, cz, shopId, isBuyer)
end
Events.OnObjectLeftMouseButtonUp.Add(onObjectLeftMouseButtonUp)

print("[Cat_ShopSystem Client] Context menu + keybind + left-click loaded. Press END to open nearest shop.")
