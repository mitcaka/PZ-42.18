...
local DEBUG_GMD = false

local function debugGMD(message)
    if DEBUG_GMD then
        print(message)
    end
end
...
local function initBanditModData(isNewGame)
    ...
    if not globalData.SurvivorZones then globalData.SurvivorZones = {} end
    if not globalData.SurvivorZoneVersion then globalData.SurvivorZoneVersion = 1 end
    if not globalData.SurvivorBaseAssignments then globalData.SurvivorBaseAssignments = {} end
    if not globalData.SurvivorLootRuns then globalData.SurvivorLootRuns = {} end
    if not globalData.SurvivorBridge then globalData.SurvivorBridge = {} end
    ...
    for i = 0, BanditClusterCount - 1 do
        ...
        debugGMD("[GMD] Cluster " .. clusterName .. " created.")
        if isClient() then
...
local function everyTenMinutes()
    if DEBUG_GMD then
        for i = 0, BanditClusterCount - 1 do
            ...
            debugGMD("[GMD] Cluster " .. i .. " is " .. cnt .. " long.")
        end
    end
end
