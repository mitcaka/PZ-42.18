...
local function banditize(zombie, bandit, clan, args)
    ...
    if args.recruitableSeeker == true then
        brain.recruitableSeeker = true
        brain.recruitableSeekerClaimed = false
    end

    -- enforcing args
    if args.hostile ~= nil then brain.hostile = args.hostile end
    if args.hostileP ~= nil then brain.hostileP = args.hostileP end
    if Bandit.IsPeacefulMode and Bandit.IsPeacefulMode() then
        brain.hostile = false
        brain.hostileP = false
    end

    if args.survivorBase then
        brain.survivorBase = args.survivorBase
        brain.survivorZoneId = args.survivorZoneId
        brain.survivorZoneRole = args.survivorZoneRole
        brain.program.name = "SurvivorBase"
        brain.program.stage = args.stage or "Prepare"
        brain.programFallback = "SurvivorBase"
        brain.hostile = false
        brain.hostileP = false
    end
...
local function spawnGroup(spawnPoints, args)
    ...
    if not clan then return 0 end

    local banditOptions = BanditCustom.GetFromClan(cid)
    if not hasEntries(banditOptions) then return 0 end
...
local function getIconDataByProgram(program, friendly)
    ...
    local roles = {
        Bandit = {icon="media/ui/raid.png", color={r=1.0, g=0.78, b=0.30}, desc="Roaming Survivors"},
        Companion = {icon="media/ui/friend.png", color={r=0.35, g=0.65, b=1.0}, desc="Companions"},
        Looter = {icon="media/ui/loot.png", color={r=1.0, g=0.85, b=0.35}, desc="Wanderers"},
        Defend = {icon="media/ui/defend.png", color={r=0.35, g=0.85, b=0.85}, desc="Defenders"},
        Camper = {icon="media/ui/tent.png", color={r=1.0, g=0.65, b=0.30}, desc="Camp"},
        Roadblock = {icon="media/ui/roadblock.png", color={r=0.75, g=0.55, b=1.0}, desc="Roadblock"},
    }

    local role = roles[program]
    if not role then return nil end

    local hostileEnabled = Bandit
        and Bandit.AreHostileSurvivorsEnabled
        and Bandit.AreHostileSurvivorsEnabled()
    if hostileEnabled and not friendly then
        return role.icon, {r=1.0, g=0.35, b=0.35}, "Hostile " .. role.desc
    end
    return role.icon, role.color, role.desc
end

local function ensureBanditCustomLoaded()
    if not BanditCustom or not BanditCustom.ClanGetAll then return false end

    local clanData = BanditCustom.ClanGetAll()
    if hasEntries(clanData) then return true end

    if BanditCustom.Load then
        BanditCustom.Load()
        clanData = BanditCustom.ClanGetAll()
    end

    return hasEntries(clanData)
end

local FREE_SURVIVOR_PRESETS = {
    civilians = {
        cid = SurvivorZones.Clan.Civilians,
        label = "civilian group",
        size = 4,
    },
    rangers = {
        cid = SurvivorZones.Clan.Rangers,
        label = "ranger patrol",
        size = 4,
    },
    police = {
        cid = SurvivorZones.Clan.PoliceBlue,
        label = "police patrol",
        size = 4,
    },
    veterans = {
        cid = SurvivorZones.Clan.Veterans,
        label = "veteran patrol",
        size = 4,
    },
}

local function getFreeSurvivorPreset(presetKey)
    return FREE_SURVIVOR_PRESETS[tostring(presetKey or "rangers")] or FREE_SURVIVOR_PRESETS.rangers
end

local function spawnFreeSurvivorPreset(player, args)
    args = args or {}
    if not player or not ensureBanditCustomLoaded() then return 0 end

    local preset = getFreeSurvivorPreset(args.preset)
    local size = math.max(1, math.min(12, math.floor(tonumber(args.size) or preset.size or 4)))
    local spawnPoints
    if args.dist then
        spawnPoints = generateSpawnPointUniform(player, tonumber(args.dist) or 60, size)
    else
        local x = math.floor(args.atPlayer == true and player:getX() or tonumber(args.x) or player:getX())
        local y = math.floor(args.atPlayer == true and player:getY() or tonumber(args.y) or player:getY())
        local z = math.floor(args.atPlayer == true and player:getZ() or tonumber(args.z) or player:getZ())
        spawnPoints = generateSpawnPointHere(player, x, y, z, size)
    end

    if not spawnPoints or #spawnPoints == 0 then return 0 end
    return spawnGroup(spawnPoints, {
        pid = BanditUtils.GetCharacterID(player),
        cid = preset.cid,
        size = #spawnPoints,
        program = "Looter",
        permanent = false,
...
local function chooseNaturalSurvivorPreset(day)
    local roll = ZombRand(100)
    if day >= 28 then
        if roll < 20 then return "civilians" end
        if roll < 45 then return "rangers" end
        if roll < 70 then return "police" end
        return "veterans"
    elseif day >= 10 then
        if roll < 40 then return "civilians" end
        if roll < 72 then return "rangers" end
        if roll < 92 then return "police" end
        return "veterans"
    end

    if roll < 55 then return "civilians" end
    if roll < 90 then return "rangers" end
    return "police"
end

local function checkNaturalSurvivorEncounter(player, day)
    local multiplier = tonumber(SandboxVars.Bandits.General_SpawnMultiplier) or 1
    local spawnChance = math.max(0, 1.5 * multiplier)
    if ZombRandFloat(0, 100) >= spawnChance then return 0 end

    local presetKey = chooseNaturalSurvivorPreset(tonumber(day) or 0)
    local preset = getFreeSurvivorPreset(presetKey)
    local size = math.max(2, (tonumber(preset.size) or 4) - 1 + ZombRand(3))
    local spawned = spawnFreeSurvivorPreset(player, {
        preset = presetKey,
        size = size,
        dist = 55 + ZombRand(10),
    })
    if spawned > 0 then
        print("[SURVIVORS] Scheduler spawned a roaming " .. tostring(preset.label) .. " of " .. tostring(spawned) .. ".")
    end
    return spawned
end

local MAX_UNCLAIMED_RECRUITABLE_SEEKERS = 2

local function countUnclaimedRecruitableSeekers()
    local count = 0
    local clusterCount = tonumber(BanditClusterCount) or 32
    for clusterId = 0, clusterCount - 1 do
        local cluster = BanditClusters and BanditClusters[clusterId] or nil
        if cluster then
            for _, brain in pairs(cluster) do
                if type(brain) == "table"
                        and brain.recruitableSeeker == true
                        and brain.recruitableSeekerClaimed ~= true then
                    count = count + 1
                end
            end
        end
...
local function spawnRecruitableSurvivor(player, args)
    args = args or {}
    if not player or not ensureBanditCustomLoaded() then return 0 end

    local spawnPoints
    if args.dist then
        spawnPoints = generateSpawnPointUniform(player, tonumber(args.dist) or 50, 1)
    else
        local x = math.floor(args.atPlayer == true and player:getX() or tonumber(args.x) or player:getX())
        local y = math.floor(args.atPlayer == true and player:getY() or tonumber(args.y) or player:getY())
        local z = math.floor(args.atPlayer == true and player:getZ() or tonumber(args.z) or player:getZ())
        spawnPoints = generateSpawnPointHere(player, x, y, z, 1)
    end
    ...
    return spawnGroup(spawnPoints, {
        cid = SurvivorZones.Clan.Veterans,
        size = 1,
        program = "RecruitableSurvivor",
        permanent = false,
        hostile = false,
        hostileP = false,
        recruitableSeeker = true,
    }) or 0
end

local function checkRecruitableSurvivorEncounter(player, day)
    if (tonumber(day) or 0) < 7 then return 0 end
    if countUnclaimedRecruitableSeekers() >= MAX_UNCLAIMED_RECRUITABLE_SEEKERS then return 0 end

    local multiplier = tonumber(SandboxVars.Bandits.General_SpawnMultiplier) or 1
    local spawnChance = math.max(0, 0.35 * multiplier)
    if ZombRandFloat(0, 100) >= spawnChance then return 0 end

    local spawned = spawnRecruitableSurvivor(player, {dist = 45 + ZombRand(15)})
    if spawned > 0 then
        print("[SURVIVORS] Scheduler spawned a recruitable armed survivor seeker.")
    end
    return spawned
end
...
local function spawnType(player, args)
    ...
    local cid = args.cid
    if not cid or not ensureBanditCustomLoaded() then return 0 end

    if LogLevel >= 3 then print ("[BANDITS] spawnType has cid " .. cid) end
    local clanData = BanditCustom.ClanGet(cid)
    local clan = clanData and clanData.spawn
    if not clan then return 0 end
    ...
    if #spawnPoints > 0 then
        cnt = spawnGroup(spawnPoints, args)
...
local function checkEvent()
    ...
    if not ensureBanditCustomLoaded() then return end
    ...
    if checkNaturalSurvivorEncounter(player, day) > 0 then
        TransmitBanditModData()
    end

    if checkRecruitableSurvivorEncounter(player, day) > 0 then
        TransmitBanditModData()
    end
end
...
BanditServer.Spawner.Clan = function(player, args)
    args = args or {}
    if not SurvivorZones.IsAdmin(player) then return 0 end
    if not args.cid or not ensureBanditCustomLoaded() then return 0 end
    args.pid = BanditUtils.GetCharacterID(player)

    if not args.size then args.size = 1 end
    args.size = math.max(1, math.min(12, math.floor(tonumber(args.size) or 1)))
    ...
    if #spawnPoints > 0 then
        return spawnGroup(spawnPoints, args)
    end
    return 0
end

local function sendSurvivorZoneSpawnerNotice(player, text)
...
BanditServer.Spawner.FreeSurvivors = function(player, args)
    args = args or {}
    if not SurvivorZones.IsAdmin(player) then return 0 end

    local preset = getFreeSurvivorPreset(args.preset)
    local spawned = spawnFreeSurvivorPreset(player, args)
    sendSurvivorZoneSpawnerNotice(player, "Spawned " .. tostring(spawned) .. " roaming survivors: " .. tostring(preset.label) .. ".")
    return spawned
end

BanditServer.Spawner.RecruitableSurvivor = function(player, args)
    args = args or {}
    if not SurvivorZones.IsAdmin(player) then return 0 end

    local spawned = spawnRecruitableSurvivor(player, args)
    sendSurvivorZoneSpawnerNotice(player, "Spawned " .. tostring(spawned) .. " recruitable armed survivor seeker.")
    return spawned
end
...
local function makeSurvivorZoneSpawnPoints(zone, count)
    local ret = {}
    local cell = getCell and getCell() or nil
    count = math.max(1, tonumber(count) or 1)

    for _ = 1, count do
        local x, y, z, square = SurvivorZones.GetRandomWalkablePoint(zone, 45)
        square = square or (cell and cell:getGridSquare(x, y, z))
        if square then
            table.insert(ret, {
                x = x,
                y = y,
                z = z,
                groundType = getGroundType(square),
                zone = getZone(square),
...
local function registerSurvivorZoneBase(zone, player)
    local gmd = GetBanditModData()
    if not gmd or not zone then return end
...
local function makeSurvivorZoneHome(zone, player, role)
    local ownerId = getPlayerId(player)
    local ownerUsername = getPlayerName(player)
    return {
        baseId = zone.baseId,
        zoneId = zone.id,
        role = role,
        ...
        mode = role == "civilian" and "civilian" or (zone.mode or "defend"),
        allowChores = role ~= "civilian" and zone.allowChores ~= false,
        x = zone.x1,
        y = zone.y1,
        x2 = zone.x2,
        y2 = zone.y2,
...
local function asClanList(value, fallback)
    local ret = {}
    if type(value) == "table" then
        for _, cid in ipairs(value) do
            if cid then table.insert(ret, cid) end
        end
    elseif value then
        table.insert(ret, value)
    end
    if fallback then table.insert(ret, fallback) end
    return ret
end

local function isUsableClan(cid)
    if not cid or not BanditCustom or not BanditCustom.ClanGet then return false end
    if not BanditCustom.ClanGet(cid) then return false end
    if BanditCustom.GetFromClan then
        local banditOptions = BanditCustom.GetFromClan(cid)
        return hasEntries(banditOptions)
    end
    return true
end

local function chooseUsableClan(zone, role)
    local profile = SurvivorZones.GetProfile(zone.type)
    local preferred
    local fallback
    if role == "civilian" then
        preferred = asClanList(zone.civilianClanIds or profile.civilianClanIds, SurvivorZones.DefaultCivilianClan)
        fallback = SurvivorZones.DefaultClan
    else
        preferred = asClanList(zone.guardClanIds or profile.guardClanIds or zone.clanId, zone.clanId or SurvivorZones.DefaultClan)
        fallback = SurvivorZones.DefaultClan
    end

    for _, cid in ipairs(preferred) do
        if isUsableClan(cid) then return cid end
    end
    if isUsableClan(fallback) then return fallback end
end

local function spawnSurvivorZoneRole(player, zone, role, count, explicitSpawnPoints)
    count = math.floor(tonumber(count) or 0)
    if count <= 0 then return 0 end

    local cid = chooseUsableClan(zone, role)
    if not cid then return 0 end

    local spawnPoints = explicitSpawnPoints or makeSurvivorZoneSpawnPoints(zone, count)
    ...
    local spawnArgs = {
        ...
        cid = cid,
        size = #spawnPoints,
        program = "SurvivorBase",
        stage = "Prepare",
        permanent = true,
        hostile = false,
        hostileP = false,
        occupation = role,
        survivorBase = makeSurvivorZoneHome(zone, player, role),
        survivorZoneId = zone.id,
        survivorZoneRole = role,
    }

    return spawnGroup(spawnPoints, spawnArgs) or 0
end

BanditServer.Spawner.SurvivorZone = function(player, args)
    args = args or {}
    local zone = SurvivorZones.Get(args.zoneId)
    if not zone and args.x and args.y then
        zone = SurvivorZones.GetNearest(tonumber(args.x), tonumber(args.y), tonumber(args.z) or 0, tonumber(args.maxDistance) or 120)
    end

    if not zone then
        sendSurvivorZoneSpawnerNotice(player, "No nearby survivor zone found.")
        return
    end

    if zone.enabled == false then
        sendSurvivorZoneSpawnerNotice(player, "Survivor zone is disabled: " .. tostring(zone.name))
        return
    end
    ...
    if not ensureBanditCustomLoaded() then
        sendSurvivorZoneSpawnerNotice(player, "Survivor profiles could not be loaded.")
        return
    end

    registerSurvivorZoneBase(zone, player)

    local role = tostring(args.role or "all")
    local guardCap = tonumber(args.size or args.guardCap or zone.guardCap or zone.populationCap) or SurvivorZones.DefaultPopulation
    local civilianCap = tonumber(args.size or args.civilianCap or zone.civilianCap) or 0
    if role == "all" then
        guardCap = tonumber(zone.guardCap or zone.populationCap) or SurvivorZones.DefaultPopulation
        civilianCap = tonumber(zone.civilianCap) or 0
    end

    local spawnedGuards = 0
    local spawnedCivilians = 0
    if role == "all" or role == "guard" then
        spawnedGuards = spawnSurvivorZoneRole(player, zone, "guard", guardCap, role == "guard" and args.spawnPoints or nil)
    end
    if role == "all" or role == "civilian" then
        spawnedCivilians = spawnSurvivorZoneRole(player, zone, "civilian", civilianCap, role == "civilian" and args.spawnPoints or nil)
    end

    local spawned = spawnedGuards + spawnedCivilians
    if spawned == 0 then
        sendSurvivorZoneSpawnerNotice(player, "No valid survivor profiles or spawn points found for this zone.")
        return
    end

    zone.lastSpawnHour = getGameTime():getWorldAgeHours()
    zone.lastSpawnCount = spawned
    zone.lastGuardSpawnCount = spawnedGuards
    zone.lastCivilianSpawnCount = spawnedCivilians
    SurvivorZones.Set(zone)
    SurvivorZones.Transmit()

    sendSurvivorZoneSpawnerNotice(player, "Survivor zone spawned " .. tostring(spawned) .. " survivors.")
end
