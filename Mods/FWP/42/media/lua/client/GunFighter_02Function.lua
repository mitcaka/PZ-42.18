...
function isLauncher(weapon)
	if	(FWPGetAmmoItemKey(weapon) == "Base.40HERound") or
		(FWPGetAmmoItemKey(weapon) == "Base.40INCRound") or
		(FWPGetAmmoItemKey(weapon) == "Base.HERocket") or
		(FWPGetAmmoItemKey(weapon) == "Base.Flare") then
		return true
	else	return false
	end
end
...
function isRocket(weapon)
	if		(FWPGetAmmoItemKey(weapon) == "Base.HERocket") then
			return true
	else	return false
	end
end
...
function isFlamer(weapon)
	if		(FWPGetAmmoItemKey(weapon) == "Base.FlameFuel") or (FWPGetAmmoItemKey(weapon) == "Base.WaterAmmo") then
			return true
	else	return false
	end
end
...
function isExtinguisher(weapon)
	if		(FWPGetAmmoItemKey(weapon) == "Base.Smoke") or (FWPGetAmmoItemKey(weapon) == "Base.PB68") then
			return true
	else	return false
	end
end
...
function isBow(weapon)
	if		(FWPGetAmmoItemKey(weapon) == "Base.Bolt_Bear") or
			(FWPGetAmmoItemKey(weapon) == "Base.Arrow_Fiberglass") or
			(FWPGetAmmoItemKey(weapon) == "Base.CrossbowBolt") or
			(FWPGetAmmoItemKey(weapon) == "Base.CrossbowBoltLarge") or
			(FWPGetAmmoItemKey(weapon) == "Base.WoodenBolt") or
			(FWPGetAmmoItemKey(weapon) == "Base.SlingShotAmmo_Rock") or
			(FWPGetAmmoItemKey(weapon) == "Base.SlingShotAmmo_Marble") then
			return true
	else	return false
	end
end
...
function isBBGun(weapon)
	if		(FWPGetAmmoItemKey(weapon) == "Base.BB177")  or (FWPGetAmmoItemKey(weapon) == "Base.PB68") then
			return true
	else	return false
	end
end
...
function isShotgun(weapon)
	if		(FWPGetAmmoItemKey(weapon) == "Base.410gShotgunShells")	or
			(FWPGetAmmoItemKey(weapon) == "Base.20gShotgunShells")	or
			(FWPGetAmmoItemKey(weapon) == "Base.ShotgunShells")		or
			(FWPGetAmmoItemKey(weapon) == "Base.10gShotgunShells")	or
			(FWPGetAmmoItemKey(weapon) == "Base.4gShotgunShells")	then
			return true
	else	return false
	end
end
...
local function FWPGetMuzzleDevice(weapon)
	if weapon == nil then
		return nil
	end
	local slots = {"Canon", "Suppressor", "suppressor", "Muzzle", "muzzle"}
	for i = 1, #slots do
		local part = nil
		if FWPGetWeaponPart then
			local okPart, valuePart = pcall(function()
				return FWPGetWeaponPart(weapon, slots[i])
			end)
			if okPart then
				part = valuePart
			end
		end
		if part == nil and weapon.getWeaponPart then
			local okWeaponPart, valueWeaponPart = pcall(function()
				return weapon:getWeaponPart(slots[i])
			end)
			if okWeaponPart then
				part = valueWeaponPart
			end
		end
		if part ~= nil then
			return part
...
local function FWPGetPartTypeText(part)
	if part == nil or not part.getType then
		return ""
	end
...
local function FWPGetSwingSoundText(weapon)
	if weapon == nil or not weapon.getScriptItem then
		return ""
	end
	local okScriptItem, scriptItem = pcall(function()
		return weapon:getScriptItem()
	end)
	if okScriptItem and scriptItem ~= nil and scriptItem.getSwingSound then
		local okSound, valueSound = pcall(function()
			return scriptItem:getSwingSound()
		end)
		if okSound and valueSound ~= nil then
			return tostring(valueSound)
		end
	end
	return ""
end
...
function isSuppressed(weapon)
	local	canon = FWPGetMuzzleDevice and FWPGetMuzzleDevice(weapon) or FWPGetWeaponPart(weapon, "Canon")
	local	canonType = FWPGetPartTypeText(canon)
	if		(canon) and (string.find(canonType, "Suppressor") or string.find(canonType, "Suppress") or string.find(canonType, "Silencer") or string.find(canonType, "Linear") or string.find(canonType, "Bottle") or string.find(canonType, "ductaped") or string.find(canonType, "OilFilter")) then
			return true
	elseif	(string.find(FWPGetSwingSoundText(weapon), "SD")) then
			return true
	else	return false
	end
end
...
function isCompensated(weapon)
	local	canon = FWPGetMuzzleDevice and FWPGetMuzzleDevice(weapon) or FWPGetWeaponPart(weapon, "Canon")
	local	canonType = FWPGetPartTypeText(canon)
	if		(canon) and (string.find(canonType, "Compensator") or string.find(canonType, "Brake") or string.find(canonType, "Linear")) then
			return true
	elseif	weapon:hasTag(FWPGetItemTag("Comp")) then
			return true
	else	return false
	end
end
...
function showMag(weapon)
	local	clip		= FWPGetWeaponPart(weapon, "Clip")
	local	MagPart		= nil
	...
	if	weapon:hasTag(FWPGetItemTag("DualMag")) then
	...
	--	FOR EVERYTHING ELSE			--
	----------------------------------
--	if	weapon:hasTag(FWPGetItemTag("Rocket")) or weapon:hasTag(FWPGetItemTag("DualMag")) then		-- Workaround for ROCKET LAUNCHER
	if	weapon:hasTag(FWPGetItemTag("Rocket")) then									-- Workaround for ROCKET LAUNCHER ONLY
...
--		DebugSay(3,"Launcher - Skip Visible Mag")
	elseif 	Loaded == true or TempLoaded == true then
			if		Ext ~= nil and Type == Ext then	MagPart = FWPCreateItem('Extended_Mag');
			elseif	Drum ~= nil and Type == Drum then	MagPart = FWPCreateItem('Drum_Mag');
			elseif	Type ~= nil then				MagPart = FWPCreateItem('Standard_Mag');
			elseif	Std ~= nil then				MagPart = FWPCreateItem('Standard_Mag');
 			end
			if		MagPart and clip == nil then
					if FWPSafeAttachWeaponPart then
						FWPSafeAttachWeaponPart(weapon, MagPart, "Clip")
					else
...
--					weapon:setWeaponPart("Mag",MagPart)
	...
	elseif	clip ~= nil then
			if		Ext ~= nil and (string.find(clip:getType(), "Extended_Mag")) then
					if FWPSafeDetachWeaponPart then
						FWPSafeDetachWeaponPart(weapon, "Clip", clip)
					else
...
--					weapon:setWeaponPart("Mag",nil)
	...
	----------------------------------
	if	weapon:hasTag(FWPGetItemTag("Rocket")) then
		if	(weapon:getCurrentAmmoCount() > 0) then
			MagPart = FWPCreateItem('Standard_Mag');
			if MagPart then
				if FWPSafeAttachWeaponPart then
					FWPSafeAttachWeaponPart(weapon, MagPart, "Clip")
				else
			...
			local rocketClip = FWPGetWeaponPart(weapon, "Clip")
			if rocketClip then
				if FWPSafeDetachWeaponPart then
					FWPSafeDetachWeaponPart(weapon, "Clip", rocketClip)
				else
					weapon:detachWeaponPart(rocketClip)
				end
			end
		end
	end
...
--	if	(weapon) and ((weapon:hasTag(FWPGetItemTag("Flex"))) or (isBow(weapon)) or (isLauncher(weapon)) or (isFlamer(weapon))) then
...
function getStoppingPower(weapon)
	local	ammo = FWPGetAmmoItemKey(weapon)
	local	value = 0
...
function getShotSound(weapon,round)
	...
	-- FWP B42 shotgun runtime sound bridge: honor normalized weapon SwingSound instead of legacy [1]Shot_* shotgun maps.
	if isShotgun(weapon) then
		if isSuppressed(weapon) then
			return "SD[1]Shot_12g"
		end
		if isCompensated(weapon) then
			return "COMP[1]Shot_12g"
		end
		local scriptSound = nil
		if weapon ~= nil and weapon.getScriptItem then
			local okScriptItem, scriptItem = pcall(function()
				return weapon:getScriptItem()
			end)
			if okScriptItem and scriptItem ~= nil and scriptItem.getSwingSound then
				...
				if okSound and valueSound ~= nil then
					scriptSound = tostring(valueSound)
				end
			end
		end
		if scriptSound ~= nil and scriptSound ~= "" and scriptSound ~= "null" and scriptSound ~= "nil" then
			return scriptSound
		end
		return "JS2000ShotgunShoot"
	end
	if	round == nil then
		round = 1
	end
...
--	if	isClient() then			-- ALLOW FOR SP ALSO
	...
	if (GUNFIGHTER.OPTIONS.options.box18 == true) then
		if	weapon:hasTag(FWPGetItemTag("ORSFX")) then
...
function getLoadedWeight(weapon)
	...
	if clip then
		...
		if mag then
			TempMag	= FWPCreateItem(mag)
			magWT		= TempMag:getWeight()
		end
	end

	if ammo then
		TempAmmo		= FWPCreateItem(ammo)
...
function emergencyReload(player, weapon)
	...
	if		Mag then
			drop = FWPCreateItem(Mag)
			if	drop:hasTag(FWPGetItemTag("Clip")) then
				Drop = 0
			end
			if	drop:hasTag(FWPGetItemTag("Fixed")) then
				Drop = 0
			end
	elseif	Mag == nil then
			Drop = 0
...
function getNextMagType(player, weapon)
	...
	if	(gun) and (instanceof(gun,"HandWeapon")) and (gun:isAimedFirearm()) and
		...
		local	result		=	FWPCreateItem(gun:getType())
	...
	--	DebugSay(2,tostring(Drum))
		...
		if (mags) and (Set ~= Clip) then
			local	tempMag		= nil
			local	tempSet		= FWPCreateItem(Set)
			local	tempStd		= FWPCreateItem(Std)
			local	tempExt		= FWPCreateItem(Ext)
			local	tempDrum	= FWPCreateItem(Drum)
					...
					for i=1, mags:size()-1 do
						if(mags:get(i)) then
							...
							if	isMagazine(mag) and (mag:getCurrentAmmoCount() > 0) then
								tempMag	= FWPCreateItem(mag:getType())
			...
			if	(next ~= nil)  and (tempMag:getType() ~= tempSet:getType()) then		-- DO NOTHING IF SAME TYPE
				TempPart = FWPCreateItem(next);				-- JUST TO GET MAXAMMO
...
local function FirearmMagazineToggle(keyNum)
	...
	if (keyNum == getCore():getKey("MAG_TYPE_1")) or (keyNum == getCore():getKey("MAG_TYPE_2")) then
		...
		if	(Gun) and (instanceof(Gun,"HandWeapon")) and (Gun:isAimedFirearm()) then
			...
			if (Gun:isContainsClip() == true) and (Gun:getMagazineType() ~= "Base.Fixed") then	-- FIRST PRESS EJECT IF NEEDED
				...
				--	======= TRANSFER CONDITION & MODE ======
					...
					if Tog then
						TempPart = FWPCreateItem(Tog);	-- JUST TO GET MAXAMMO
		...
		elseif	(Gun) and (instanceof(Gun,"HandWeapon")) then	-- MELEE WEAPON W/MULTI GRIP
			...
			if	Gun:hasTag(FWPGetItemTag("CanIgnite")) then
				if	(Second) and (Second:hasTag(FWPGetItemTag("StartFire"))) then
			...
			if	grip == nil then
				...
				local result = FWPCreateItem(grip);
	...
	elseif (keyNum == getCore():getKey("AMMO_TYPE_1")) or (keyNum == getCore():getKey("AMMO_TYPE_2")) then	-- [<] or [>]
		...
		----------------------------------
		if (Gun) and (instanceof(Gun,"HandWeapon")) and (Gun:isAimedFirearm()) then
			local	sling		= FWPGetWeaponPart(Gun, "Sling")
			local	Melee		= Gun:getModData().Melee
			local	Integral	= Gun:getModData().Integral
			local	hasLauncherModes	= (Gun:getModData().HEMode ~= nil) or (Gun:getModData().INCMode ~= nil)
			if 	( (sling) and (string.find(sling:getType(), "Launcher")) and (not isLauncher(Gun)) ) or
				( (Integral) and (Integral == "Launcher") and (not isLauncher(Gun)) ) or
				( hasLauncherModes and (not isLauncher(Gun)) ) then
			...
			--	end
				...
				if launcher == nil then
					-- FWP launcher mode switch skipped because this weapon has no matching HE/INC transform.
					DebugSay(2,getText("ContextMenu_NoFunction"))
					return
				end

				if FWPRequestWeaponTransform then
					local ok = FWPRequestWeaponTransform(player, Gun, launcher, "launcherMode", { hotbarWeight = 1 })
					if ok then return end
				end

				local result = FWPCreateItem(launcher);
			...
			-- 	[GUN to LAUNCHER] SAVE ATTACHMENT TO ModData.TempXXX
				...
				if	Gun:getModData().TempAmmoType == 1 and FWPGetAmmoItemKey(result) == "Base.40INCRound" then	-- HE LOADED, [GUN to INC LAUNCHER]
					for i=1, Remove do
				...
				elseif	Gun:getModData().TempAmmoType == 2 and FWPGetAmmoItemKey(result) == "Base.40HERound" then	-- INC LOADED, [GUN to HE LAUNCHER]
					for i=1, Remove do
				...
				-- COMPATIBLE AMMO				--
				----------------------------------
				local	comp		= FWPGetCompatibleAmmoTransform and FWPGetCompatibleAmmoTransform(Gun) or Gun:getModData().CompAmmo
				local	Integral	= Gun:getModData().Integral
				...
				elseif	(comp) and (Gun:isRoundChambered() == true or Gun:getCurrentAmmoCount() > 0) then		-- FIRST UNLOAD OTHER AMMO
						if	(Gun:getCurrentAmmoCount() > 0) and ISUnloadBulletsFromFirearm then
				...
				elseif	(comp ~= nil) then
						...
						if FWPRequestWeaponTransform then
							local ok = FWPRequestWeaponTransform(player, Gun, comp, "compatibleAmmo", { hotbarWeight = 1 })
							if ok then return end
						end
						local result = FWPCreateItem(comp);
						if result == nil then
							DebugSay(2,getText("ContextMenu_NoFunction"))
							return
						end
	...
	elseif (keyNum == getCore():getKey("TRAJECTORY_DOWN")) or (keyNum == getCore():getKey("TRAJECTORY_UP")) then	-- [-] or [+]
		...
		-- GRENADE TRAJECTORY			--
		----------------------------------
	--	if	(Gun) and (isFlamer(Gun) or isLauncher(Gun) or Gun:hasTag(FWPGetItemTag("PCP"))) and		-- ADD PAINTBALL GUN
		...
		--	local scope		= FWPGetWeaponPart(Gun, "Scope")
		--	if	(scope) then
	...
	elseif (keyNum == getCore():getKey("FOLD_STOCK")) then	-- [?]
		...
		if (Gun) and (instanceof(Gun,"HandWeapon")) and (Gun:isAimedFirearm()) then
			...
			if	fold ~= nil then
				...
				if FWPRequestWeaponTransform then
					local ok = FWPRequestWeaponTransform(player, Gun, tog, "fold", { hotbarWeight = 30 })
					if ok then return end
				end

				local result = FWPCreateItem(tog);
				player:getInventory():AddItem(result);
...
--	end
	...
	if (keyNum == getCore():getKey("SELECT_FIRE")) then	-- [BACKSPACE] key
		...
		elseif	(GUNFIGHTER.OPTIONS.options.box131 == true) then
			if	(Gun) and (Gun:hasTag(FWPGetItemTag("Thrown"))) then
...
local function WeaponReloadScript(keyNum)
	...
	if (keyNum == getCore():getKey("REFRESH_SCRIPT")) then
		...
		if (Gun) and (instanceof(Gun,"HandWeapon")) and (Gun:isAimedFirearm()) then
			...
			local result	= FWPCreateItem(clone);
			player:getInventory():AddItem(result);
		...
		--	====== TRANSFER MAG-TYPE =====
			...
			if result:getMagazineType() ~= nil then
					result:setMaxAmmo(FWPCreateItem(result:getMagazineType()):getMaxAmmo())
...
--  MELEE MODE TOGGLE													--
--------------------------------------------------------------------------
local function FWPIsFirearmBackedMeleeMode(weapon)
	...
	return weapon:isAimedHandWeapon() and weapon:getModData().Melee ~= nil
end

local function FWPTransferMeleeModeAttachments(Gun, result)
	if not (Gun and result and instanceof(Gun, "HandWeapon") and instanceof(result, "HandWeapon")) then
		return
	end

	local resultMd = result:getModData()
	local gunMd = Gun:getModData()

	if result:isAimedFirearm() and Gun:isAimedFirearm() then
		result:attachWeaponPart(Gun:getWeaponPart("Scope"))
		result:attachWeaponPart(Gun:getWeaponPart("Canon"))
		result:attachWeaponPart(Gun:getWeaponPart("Clip"))
		result:attachWeaponPart(Gun:getWeaponPart("Stock"))
		result:attachWeaponPart(Gun:getWeaponPart("Sling"))
		result:attachWeaponPart(Gun:getWeaponPart("RecoilPad"))
	elseif (not Gun:isAimedFirearm()) and result:isAimedFirearm() then
		result:attachWeaponPart(gunMd.TempScope)
		result:attachWeaponPart(gunMd.TempCanon)
		result:attachWeaponPart(gunMd.TempClip)
		result:attachWeaponPart(gunMd.TempStock)
		result:attachWeaponPart(gunMd.TempSling)
		result:attachWeaponPart(gunMd.TempRecoilPad)
	elseif Gun:isAimedFirearm() and (not result:isAimedFirearm()) then
		resultMd.TempScope = Gun:getWeaponPart("Scope")
		resultMd.TempCanon = Gun:getWeaponPart("Canon")
		resultMd.TempClip = Gun:getWeaponPart("Clip")
		resultMd.TempStock = Gun:getWeaponPart("Stock")
		resultMd.TempSling = Gun:getWeaponPart("Sling")
		resultMd.TempRecoilPad = Gun:getWeaponPart("RecoilPad")
		result:attachWeaponPart(resultMd.TempScope)
		result:attachWeaponPart(resultMd.TempCanon)
		result:attachWeaponPart(resultMd.TempClip)
		result:attachWeaponPart(resultMd.TempStock)
		result:attachWeaponPart(resultMd.TempSling)
		result:attachWeaponPart(resultMd.TempRecoilPad)
	end
end

local function FWPGetLauncherNormalFireTarget(weapon)
	if not (weapon and weapon.getModData) then
		return nil
	end
	local md = weapon:getModData()
	local target = md.FWPLauncherReturnType
	if target == nil and FWPGetScriptItemProperty then
		target = FWPGetScriptItemProperty(weapon, "Melee")
	end
	if target ~= nil and FWPNormalizeFullType then
		target = FWPNormalizeFullType(target)
	end
	return target
end

local function FWPIsNormalFireModeKey(keyNum)
	local core = getCore and getCore() or nil
	return core and core.getKey and keyNum == core:getKey("NORMAL_FIRE_MODE")
end

local function WeaponNormalFireModeToggle(keyNum)
	if getSpecificPlayer(0) == nil then return end
	if not FWPIsNormalFireModeKey(keyNum) then return end

	local player = getSpecificPlayer(0)
	local Gun = player:getPrimaryHandItem()
	if not (Gun and instanceof(Gun, "HandWeapon") and isLauncher(Gun)) then return end

	local target = FWPGetLauncherNormalFireTarget(Gun)
	if target == nil then
		DebugSay(2,getText("ContextMenu_NoFunction"))
		return
	end

	player:getEmitter():playSound("stormyRifleInsertRound")
	DebugSay(3,"Normal Fire Mode")

	if FWPRequestWeaponTransform then
		FWPRequestWeaponTransform(player, Gun, target, "normalFireMode", { hotbarWeight = 1 })
	end
end
...
local function WeaponMeleeToggle(keyNum)
	...
	if (keyNum == getCore():getKey("MELEE_MODE")) then	-- [/]

		local tog		= nil
		local player	= getSpecificPlayer(0)
		local Gun		= player:getPrimaryHandItem()

		if (Gun) and (instanceof(Gun,"HandWeapon")) and isLauncher(Gun) then
			DebugSay(2,"Use Normal Fire Mode")
			return
		end

		if (Gun) and FWPIsFirearmBackedMeleeMode(Gun) then
			local Melee	= Gun:getModData().Melee
			local Integral	= Gun:getModData().Integral
			local sling	= FWPGetWeaponPart(Gun, "Sling")
			--------------------------------------
			...
			--	end
				...
				if FWPRequestWeaponTransform then
					local ok = FWPRequestWeaponTransform(player, Gun, tog, "meleeMode", { hotbarWeight = 1 })
					if ok then return end
				end
			...
			-- AMMO TYPE DATA
				...
				if 	(Gun:getCurrentAmmoCount() > 0) or (Gun:isRoundChambered()) or (Gun:isContainsClip() == true) then						-- STORE GRENADE TYPE IN M16 TEMP
					if	FWPGetAmmoItemKey(Gun) == "Base.40HERound" then		-- USE AMMOTYPE INSTEAD
			...
			--------------------------------------
				...
				if tog == nil then
					...
					if FWPRequestWeaponTransform then
						local ok = FWPRequestWeaponTransform(player, Gun, tog, "meleeMode", { hotbarWeight = 10 })
						if ok then return end
					end
				...
				--	====== TRANSFER WEAPON ATTACHMENTS SO THEY DO NOT AFFECT MELEE VERSION
				FWPTransferMeleeModeAttachments(Gun, result)
			...
			--		local	minDmg		= result:getMinDamage()		-- USE MODIFIED DAMAGE
					...
					local	canon		= FWPGetWeaponPart(result, "Canon")
					local	scope		= FWPGetWeaponPart(result, "Scope")
					local	bayo		= 0
...
local function WeaponNightVisionToggle(keyNum)
	...
	local nvctrl = FWPGetNVAPIController()
			...
			for i=1, items:size()-1 do
				local item = items:get(i)
					if	(item ~= nil) and (item:getCategory() == "Clothing") and
						(item:hasTag(FWPGetItemTag("NVITEM"))) and (item ~= weapon) and
						(item:isEquipped()) and
...
function isWeaponNightVision(weapon)
	if (weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) then
		scope	= FWPGetWeaponPart(weapon, "Scope")
		if 	(scope) and (scope:hasTag(FWPGetItemTag("NVITEM"))) then
			return true
		else	return false
		end
	else	return false
...
local function WeaponNightVision()
	...
	if 	(scope) and (scope:hasTag(FWPGetItemTag("NVITEM"))) then
		...
		local	NV_Temp = FWPCreateItem('NV_Control')
...
--------------------------------------------------------------------------
local function FWPWeaponLightSlotForPart(weapon, part)
	if not (weapon and part) then return nil end
	local stock = FWPGetWeaponPart(weapon, "Stock")
	if stock == part then return "Stock" end
	local sling = FWPGetWeaponPart(weapon, "Sling")
	if sling == part then return "Sling" end
	return nil
end

local function FWPGetSavedWeaponLightCharge(weapon, slot)
	...
	if charge > 100 then return 100 end
	return charge
end

local function FWPGetWeaponLightPartCharge(weapon, part, slot)
	if not part then return nil end
	slot = slot or FWPWeaponLightSlotForPart(weapon, part)
	local partCharge = nil
	if part.getModData then
		partCharge = tonumber(part:getModData().Charge)
	end
	local savedCharge = FWPGetSavedWeaponLightCharge(weapon, slot)
	if (partCharge == nil or partCharge <= 0) and savedCharge ~= nil and savedCharge > 0 then
		if part.getModData then
			part:getModData().Charge = savedCharge
		end
		return savedCharge
	end
	return partCharge or savedCharge
end

local function FWPSetWeaponLightPartCharge(weapon, part, slot, charge)
	slot = slot or FWPWeaponLightSlotForPart(weapon, part)
...
local function WeaponLightChargeData(player, weapon)
	...
	if	(weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) then
		...
		----------------------------------
		if		stock and ( stock:hasTag(FWPGetItemTag("Light")) or stock:hasTag(FWPGetItemTag("Laser")) ) then
				local	partCharge = tonumber(stock:getModData().Charge)
				local	saveCharge = tonumber(weapon:getModData().Charge)
				if		(partCharge == nil or partCharge <= 0) and saveCharge ~= nil and saveCharge > 0 then
			...
			--			DebugSay(2,"Charge Data Transfered - "..tostring(round(stock:getModData().Charge,1) ) )
				...
				elseif	partCharge ~= nil and saveCharge == nil then
						weapon:getModData().Charge = partCharge
		...
		----------------------------------
		if		sling and ( sling:hasTag(FWPGetItemTag("Light")) or sling:hasTag(FWPGetItemTag("Laser")) ) then
				local	partCharge = tonumber(sling:getModData().Charge)
				local	saveCharge = tonumber(weapon:getModData().Charge2)
				if		(partCharge == nil or partCharge <= 0) and saveCharge ~= nil and saveCharge > 0 then
...
local function WeaponLightToggle(keyNum)
	...
	if (keyNum == getCore():getKey("WEAPON_LIGHT")) or (keyNum == getCore():getKey("WEAPON_BIPOD")) then
		...
		if FWPIsNormalFireModeKey(keyNum) and weapon and instanceof(weapon,"HandWeapon") and isLauncher(weapon) then
			return
		end
		...
		if		(weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) then
				stock	= FWPGetWeaponPart(weapon, "Stock")
				sling	= FWPGetWeaponPart(weapon, "Sling")

				if		(stock) and ( (string.find(stock:getType(), "Light")) or (stock:hasTag(FWPGetItemTag("Light"))) ) then
						hasLight = stock
						local partCharge = FWPGetWeaponLightPartCharge(weapon, stock, "Stock")
						if (partCharge and partCharge > 0) then
							charge = partCharge
						end
				end
				if		(stock) and ( (string.find(stock:getType(), "Laser")) or (stock:hasTag(FWPGetItemTag("Laser"))) ) then
						hasLaser = stock
						local partCharge = FWPGetWeaponLightPartCharge(weapon, stock, "Stock")
						if (partCharge and partCharge > 0) then
							charge = partCharge
				...
				------------------------------
				if		(sling) and ( (string.find(sling:getType(), "Light")) or (sling:hasTag(FWPGetItemTag("Light"))) ) then
						hasLight = sling
						local partCharge = FWPGetWeaponLightPartCharge(weapon, sling, "Sling")
						if (partCharge and partCharge > 0) then
							charge = partCharge
						end
				end
				if		(sling) and ( (string.find(sling:getType(), "Laser")) or (sling:hasTag(FWPGetItemTag("Laser"))) ) then
						hasLaser = sling
						local partCharge = FWPGetWeaponLightPartCharge(weapon, sling, "Sling")
						if (partCharge and partCharge > 0) then
							charge = partCharge
				...
				if stock ~= nil then
					togStock = FWPCreateItem(stock:getModData().Mode_Toggle)
				end
				if sling ~= nil then
					togSling = FWPCreateItem(sling:getModData().Mode_Toggle)
				end
		end

		if (keyNum == getCore():getKey("WEAPON_LIGHT")) then					-- CONDITION
		
		--	DebugSay(2,"part Laser TAG - "..tostring(stock:hasTag(FWPGetItemTag("Laser"))))
		--	DebugSay(2,"part Light TAG - "..tostring(stock:hasTag(FWPGetItemTag("Light"))))
		...
		--		DebugSay(2,"Charge "..tostring(charge))
				...
				if		attacker:isAiming() and hasLaser and (GUNFIGHTER.OPTIONS.options.dropdown144 <= 2) and charge and charge > 0 then
						...
						--	DebugSay(2,"Toggling..."..tostring(togStock:getType()))
							...
							--	TRANSFER CHARGE	--
							----------------------
							togStock:getModData().Charge = FWPGetWeaponLightPartCharge(weapon, hasLaser, "Stock") or 0
		...
		elseif (keyNum == getCore():getKey("WEAPON_BIPOD")) then
			...
			--	DebugSay(2,"INTEGRAL FOLD2")
				...
				if FWPRequestWeaponTransform then
					local ok = FWPRequestWeaponTransform(attacker, weapon, tog, "fold2", { hotbarWeight = 30 })
					if ok then return end
				end
			...
			--	====== TRANSFER MAG-TYPE =====
				...
				elseif	result:getMagazineType() == "Base.Fixed" then			-- FIXED MAG GUNS... SEEMS BACKWARDS ???
					DebugSay(3,"...")
				elseif	weapon:getMagazineType() ~= "Base.Fixed" then			-- MAG FED GUNS
...
local function WeaponLightBeam()
	...
	if		(stock) and ( (string.find(stock:getType(), "Light")) or (stock:hasTag(FWPGetItemTag("Light"))) ) then
			light = stock
			charge = FWPGetWeaponLightPartCharge(weapon, light, "Stock")
	end
	if		(stock) and ( (string.find(stock:getType(), "Laser")) or (stock:hasTag(FWPGetItemTag("Laser"))) ) then
			laser = stock
			charge = FWPGetWeaponLightPartCharge(weapon, laser, "Stock")
	end
	if		(sling) and ( (string.find(sling:getType(), "Light")) or (sling:hasTag(FWPGetItemTag("Light"))) ) then
			light = sling
			charge = FWPGetWeaponLightPartCharge(weapon, light, "Sling")
	end
	if		(sling) and ( (string.find(sling:getType(), "Laser")) or (sling:hasTag(FWPGetItemTag("Laser"))) ) then
			laser = sling
			charge = FWPGetWeaponLightPartCharge(weapon, laser, "Sling")
	end
...
--	end
	...
	--------------------------
	if	(laser) then
		local laserSlot = FWPWeaponLightSlotForPart(weapon, laser)
		if stock ~= nil then
			togStock = FWPCreateItem(stock:getModData().Mode_Toggle)
		end
		--------------------------
		--	SHUTDOWN IF EMPTY	--
		--------------------------		
		if	laserSlot == "Stock" and stock and togStock and weapon:getModData().LaserOn == false and (string.find(stock:getType(), "_ON")) then
			togStock:getModData().Charge = FWPGetWeaponLightPartCharge(weapon, laser, laserSlot) or 0
			weapon:attachWeaponPart(togStock)
			checkHotbar(attacker, weapon, weapon, 1)			-- (30) CHECK HOTBAR
			ReEquipIt(attacker, weapon)
			DebugSay(2,getText("ContextMenu_Laser_OFF"))
		...
		--	AUTO WHEN AIM		--
		--------------------------
		if	(GUNFIGHTER.OPTIONS.options.dropdown144 >= 3) and laserSlot == "Stock" and stock then
			local autotog = nil
			if weapon:getModData().LaserOn == nil then
				weapon:getModData().LaserOn = false
			end
		...
		--------------------------
		if	weapon:getModData().LaserOn == true then
			charge = FWPGetWeaponLightPartCharge(weapon, laser, laserSlot)
			if	charge ~= nil then
				if	charge > 0 then
					FWPSetWeaponLightPartCharge(weapon, laser, laserSlot, charge - (getWeaponLightRuntime() * laser:getModData().Runtime))
	...
	if	(weapon) and (light) and (weapon:getModData().LightOn == true) then
		local lightSlot = FWPWeaponLightSlotForPart(weapon, light)
		charge = FWPGetWeaponLightPartCharge(weapon, light, lightSlot)
		--------------------------
		--	DRAIN BATTERY		--
		--------------------------
		if	charge ~= nil then
			if	charge > 0 then
				FWPSetWeaponLightPartCharge(weapon, light, lightSlot, charge - (getWeaponLightRuntime() * light:getModData().Runtime))
	...
	--	NIGHT VISION		--
	--------------------------
	elseif	(scope) and (scope:hasTag(FWPGetItemTag("NVITEM"))) then				-- CHECK NV LAST SO LIGHT PREVAILS
	...
	--	LIGHT SABER	PRIMARY	--
	--------------------------
	elseif	(weapon) and (weapon:hasTag(FWPGetItemTag("LightSaber")) or weapon:hasTag(FWPGetItemTag("Torch"))) then
		if	(weapon:getCondition() >= 1) then
			...
			if weapon:hasTag(FWPGetItemTag("LightSaber")) then
...
--			Glow = attacker:getModData().Glow
		...
		if	(weapon:hasTag(FWPGetItemTag("Torch"))) or (weapon:hasTag(FWPGetItemTag("LightSaber"))) then
...
--			attacker:getModData().Glow = nil
	...
	------------------------------
	if	(second) and (second:hasTag(FWPGetItemTag("LightSaber")) or second:hasTag(FWPGetItemTag("Torch"))) then
		if	(second:getCondition() >= 1) then
			...
			if not second:hasTag(FWPGetItemTag("Flex")) then
				SpriteA = second:getModData().SpriteA
			end

			local Condition	= second:getCondition()
			if second:hasTag(FWPGetItemTag("LightSaber")) then
				Condition = Condition / 10
			end
...
--			Glow2 = attacker:getModData().Glow2
		...
		if	(second:hasTag(FWPGetItemTag("Torch"))) or (second:hasTag(FWPGetItemTag("LightSaber"))) then
...
function TorchCondition(player, weapon)
	...
	--	DebugSay(2,tostring(weapon:getDisplayName()).." - "..tostring(weapon:getCondition()) )
		if weapon:getCondition() <= 1 and off then	-- 1 SO NO DROP OR AUTO-BREAK
			...
			local tempOff = FWPCreateItem(off)
...
--		local attacker	= getSpecificPlayer(0)
		...
		if 	(weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedHandWeapon()) then
			stock	= FWPGetWeaponPart(weapon, "Stock")
			scope	= FWPGetWeaponPart(weapon, "Scope")
			canon	= FWPGetWeaponPart(weapon, "Canon")
...
function BreakAttachmentOnFire(player, weapon)
	if weapon and instanceof(weapon,"HandWeapon") and weapon:isAimedFirearm() and isShotgun and isShotgun(weapon) then
		-- FWP B42 shotgun legacy fire hook guard: B42 owns shotgun trigger/chamber/projectile state.
		return
	end
	----------------------
	--  OPTIONS BOX 12  --
...
function GunFighterInventoryScan()
	...
	if items then
		for i = 1, items:size()-1 do
			local item = items:get(i)
			if	(item ~= nil) then 
				if	(item:hasTag(FWPGetItemTag("Null"))) then
		...
		for i = 1, items:size()-1 do
			...
			if	(item ~= nil) then 
				...
				elseif (instanceof(item,"HandWeapon")) and (item:isAimedFirearm()) and (not isLauncher(item)) then
					...
					--	if		(string.find(FWPGetWeaponPart(item, "Clip"):getType(), "Standard_Mag"))	and (item:getMagazineType() ~= nil) then
					--			resetMag = item:getMagazineType()
						if		(string.find(FWPGetWeaponPart(item, "Clip"):getType(), "Extended_Mag"))	and (item:getModData().ExtMagType ~= nil) then
								DebugSay(2,"[CONTAINER ITEM] Extended Mag Detected...")
								resetMag = item:getModData().ExtMagType
						elseif	(string.find(FWPGetWeaponPart(item, "Clip"):getType(), "Drum_Mag"))		and (item:getModData().DrumMagType ~= nil) then
...
function CheckFixedWeapon()		-- (player, weapon)
	...
	if player:getModData().Fix_Loc == nil then		-- ONLY CHECK IF NOT FIXED
		...
		for i = 1, items:size()-1 do
			local item = items:get(i)
			if (item ~= nil) and (item:hasTag(FWPGetItemTag("Fixed"))) and (instanceof(item,"HandWeapon")) and (item:isAimedFirearm()) then
			...
			-- WHEN FOLDED		--
			----------------------
			elseif (item:hasTag(FWPGetItemTag("Fixed_FOLD"))) then
...
--				DebugSay(2,"MOVEMENT RESTORED...")
		...
		if	( attacker:getModData().Fix_Item == nil or attacker:getModData().Fix_Loc == nil or not attacker:getPrimaryHandItem():hasTag(FWPGetItemTag("Fixed")) ) then
			if	attacker:getSlowFactor() == 1 then
				attacker:setSlowFactor(0)
				attacker:setSlowTimer(0)
			--	DebugSay(2,"SLOW FACTOR - "..tostring(attacker:getSlowFactor()))	
...
--		attacker:Say(tostring(attacker:getBeenMovingFor()))		TESTING FOR SLOW ROF
	...
	elseif	weapon and weapon:hasTag(FWPGetItemTag("PCP")) then
...
--	if		getSpecificPlayer(0):getVehicle()		then	stock	= 0	-- NO STOCK BUFF IN VEHICLE
	...
	if weapon:isRanged() then
		...
		if		weapon:getFireMode() == "[6]Rotary" then	-- GIVE MINIGUNS STABILITY BUFF
				...
				base	= 22
		elseif	weapon:hasTag(FWPGetItemTag("Fixed")) then					-- GIVE FIXED GUNS STABILITY BUFF
				recoil	= (recoil + 50)  / 5				-- RECOIL MATTER WAY LESS
				weight	= (weight + 300) / 20				-- WEIGHT MATTER LESS
				base	= 20
		end

		if	weapon:hasTag(FWPGetItemTag("Grip")) then	recoil = recoil - 2 end				-- SCRIPTED INTEGRAL FOREGRIP
		if	weapon:hasTag(FWPGetItemTag("Comp")) then	recoil = recoil - 2 end				-- SCRIPTED INTEGRAL FOREGRIP
...
-- TEST LAST SPOTTED
	...
	--	end
		...
		if weapon ~= nil and (instanceof(weapon,"HandWeapon")) and (weapon:isRanged() or weapon:hasTag(FWPGetItemTag("Thrown"))) then

		----------------------
		--  OPTIONS BOX 4  --
		----------------------
		if (GUNFIGHTER.OPTIONS.options.box4 == true) and (FWPGetAmmoItemKey(weapon) ~= nil) then
			local r2 = 0.8
			local g2 = 0.8
			local b2 = 0.8
		...
		--	end
			...
			--	DebugSay(3,"X "..tostring(sx).." Y"..tostring(sy).."ZM "..tostring(zoom))
				...
				if weapon:isJammed() then
					...
					g = 0.0
					b = 0.0
				elseif (not weapon:hasTag(FWPGetItemTag("Thrown"))) and ( (weapon:haveChamber() == false and weapon:getCurrentAmmoCount() == 0) or (weapon:haveChamber() ~= false and weapon:isRoundChambered() == false) ) then
					r = 1.0
					g = 1.0
					b = 1.0
				end
...
Events.OnGameStart.Add(function()
	if not FWP_DISABLE_GUNFIGHTER_HUD then
...
--		(not isFlamer(weapon)) and (not isBBGun(weapon)) and (not isBow(weapon)) then
		...
		if (GUNFIGHTER.OPTIONS.options.dropdown19 > 1) then		-- 1 EQUALS NO BRASS EJECTION
			...
			local catch		= nil
			if	weapon:getWeaponPart("RecoilPad") and weapon:getWeaponPart("RecoilPad"):hasTag(FWPGetItemTag("Brass_Catch")) then
...
local function rangePCP(player, target, weapon, damage)
	...
	if	(weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) and player:isAiming() and
		(weapon:hasTag(FWPGetItemTag("PCP"))) then
...
local function setOnFire(player, target, weapon, damage)
	if	(weapon) and (weapon:hasTag(FWPGetItemTag("Torch"))) and (GUNFIGHTER.OPTIONS.options.dropdown138 ~= 0) then
...
local function setStuckProjectile(target, player, BodyPart, weapon)
	...
	if	(weapon) and ( (weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon)) or weapon:hasTag(FWPGetItemTag("Thrown")) ) then	-- NOT FIREARMS
	...
	--	local back		= true				-- TEST BACK STICK LOCATIONS
		...
		local	projectile = FWPCreateItem(stuck);
		if	cond then
		...
		if	(stuck == ammo) and (ZombRand(bust) == 2) then				-- ROLL 10 to 100
			if (projectile:getModData().Break) ~= nil then				-- AMMO MUST HAVE BROKEN STAT
				projectile = FWPCreateItem(projectile:getModData().Break);
			end
		end
		...
		if	(bp == "Head") or (bp == "Neck") then
			...
			if		(Gun:hasTag(FWPGetItemTag("Spear")) and back == false) or (not Gun:hasTag(FWPGetItemTag("Spear")) and back == true) then
...
local function startProjectileTimer(player, weapon)
	...
	if	(weapon) and ((weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon)) or weapon:hasTag(FWPGetItemTag("Thrown"))) then
		...
		if (weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon)) and weapon:getCurrentAmmoCount() > 0 then
			player:getModData().TempThrown = FWPGetAmmoItemKey(weapon)
		elseif weapon:hasTag(FWPGetItemTag("Thrown")) then
...
function getNextThrown(player, weapon)
	...
	if weapon and weapon:hasTag(FWPGetItemTag("Thrown")) and player:getModData().AutoThrow ~= nil then		-- ONLY DO IF THROWN IS USED
		local inv		= player:getInventory()
...
--		if	(weapon) and (weapon:hasTag(FWPGetItemTag("Thrown"))) and	-- SEC BACK INVENTORY
--			(sec) and (sec:hasTag(FWPGetItemTag("Thrown"))) then		-- BUT ONLY IF THROWING
		...
		if (items) then
			if	(throw == nil) then
				for i=1, items:size()-1 do				-- SECONDARY HAND FIRST
					if(items:get(i)) then
						local item = items:get(i)
			--			if		(throw == nil) and (sec) and (item:getType() == sec:getType()) and (item:hasTag(FWPGetItemTag("Thrown"))) then
						if		(throw == nil) and (sec) and (item == sec) and (item:hasTag(FWPGetItemTag("Thrown"))) then
								equip = 10
							--	throw = item
								Second = item
						end
			...
			if	(throw == nil) then
				for i=1, items:size()-1 do
					if(items:get(i)) then
						local item = items:get(i)
						if		(throw == nil) and (item:getAttachedSlot() == 3) and (item:hasTag(FWPGetItemTag("Thrown"))) then
								equip = 15
							--	throw = item
								RBelt = item
						end
			...
			if	(throw == nil) then
				for i=1, items:size()-1 do
					if(items:get(i)) then
						local item = items:get(i)
						if		(throw == nil) and (item:getAttachedSlot() == 2) and (item:hasTag(FWPGetItemTag("Thrown"))) then
								equip = 20
							--	throw = item
								LBelt = item
						end
			...
			if	(throw == nil) then
				for i=1, items:size()-1 do
					if(items:get(i)) then
						local item = items:get(i)
						if		(throw == nil) and (item:getAttachedSlot() == 1) and (item:hasTag(FWPGetItemTag("Thrown"))) then
								equip = 25
							--	throw = item
								Back = item
						end
			...
			if	(throw == nil) then			-- THEN LOOK FOR ANY THROWN
				for i=1, items:size()-1 do
					if(items:get(i)) then
						local item = items:get(i)
						if		(throw == nil) and (item:hasTag(FWPGetItemTag("Thrown"))) then
								equip = 80
							--	throw = item
								Next = item
						end
...
local function checkProjectileTimer(player)
	...
	if		weapon and (weapon:hasTag(FWPGetItemTag("Thrown")) or (weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon))) and weapon:getMaxRange() then
...
--	elseif	(player:getModData().ProjectileTimer == 0) and (player:getModData().ProjectileHit == 0) then
	if	(player:getModData().ProjectileTimer <= getTimestamp()) and (player:getModData().ProjectileHit == 0) then
		...
		--		if	(projectile) and projectile:hasTag(FWPGetItemTag("Thrown")) and (projectile:getMaxHitCount() == 0) then
		...
		--		end
				...
				--------------------------
				local	bust = (GUNFIGHTER.OPTIONS.options.dropdown135 * 2)		-- 1 IS NEVER / 10 MEANS 10%
				if	(stop ~= fwd) and (ZombRand(bust) == 2) and (weapon) and (weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon)) then	-- WHEN THROWN, THERES NO WEAPON!!!
					local Broken = FWPCreateItem(projectile)
...
function AutoTransform(player)
	...
	if	(player:getModData().TimeSprite) and (player:getModData().TimeSprite <= 0) and (weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:getWeaponSprite() == "null") and player:getModData().TempSprite ~= nil then
		...
		if (instanceof(weapon,"HandWeapon")) then
			if FWPRestoreWeaponSprite then
				FWPRestoreWeaponSprite(weapon, player:getModData().TempSprite)
			else
	...
	if (weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) then
		local	scriptItem = weapon:getScriptItem()
		local	scope = FWPGetWeaponPart(weapon, "Scope")
		...
		if	isRocket(weapon) then
			...
			Armed	= FWPGetWeaponPart(weapon, "Clip")
		...
		--  LAUNCHER PROJECTILE			--
		----------------------------------
		if  ((weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon))) or
            ((not weapon:isAimedHandWeapon()) and
	...
	--			DebugSay(4,"Attachment... No Transform")
			...
			----------------------------------
			if		(weapon:hasTag(FWPGetItemTag("XBow")) or isBow(weapon)) then
	--		elseif	weapon:hasTag(FWPGetItemTag("XBow")) then
					local spriteNorm = weapon:getModData().SpriteNORM
                    local spriteFire = weapon:getModData().SpriteFIRE
                    if (spriteNorm == nil or spriteFire == nil) and scriptItem and scriptItem.getProperty then
                        if spriteNorm == nil then
                            spriteNorm = scriptItem:getProperty("SpriteNORM")
                        end
                        if spriteFire == nil then
                            spriteFire = scriptItem:getProperty("SpriteFIRE")
                        end
                    end

                    if      (weapon:getCurrentAmmoCount() > 0) and spriteFire and weapon:getWeaponSprite() ~= spriteFire then -- IS LOADED
                            weapon:setWeaponSprite(spriteFire)
                            ReEquipIt(attacker,weapon)
                    elseif  (weapon:getCurrentAmmoCount() == 0) and spriteNorm and weapon:getWeaponSprite() ~= spriteNorm then -- NOT LOADED
                            weapon:setWeaponSprite(spriteNorm)
                            ReEquipIt(attacker,weapon)
                    end
			...
			elseif	isFlamer(weapon) or isExtinguisher(weapon) or isRocket(weapon) or isLauncher(weapon) then
				...
				if	attacker:isAiming() then
					...
					local proj = nil
					if ammo then
						local tempAmmo = FWPCreateItem(ammo)
						if tempAmmo then
							------------------------------
							-- AMMO WITH SWITCH TYPES	--
							------------------------------
							if	tempAmmo:getModData().Fired then
								local firedAmmo = FWPCreateItem(tempAmmo:getModData().Fired)
								if firedAmmo then
			...
			--		DebugSay(2,"Proj - "..tostring(proj))
				...
				--	NOT AIMING		--
				----------------------
				else	local revert = FWPCreateItem(weapon:getType())
...
--					elseif	(current <= 1) or (count + BurstShots < current) or (current < count) or (count == current) then
	...
	----------------------------------
	elseif (weapon and not weapon:isBroken() and weapon:hasTag(FWPGetItemTag("Flex"))) then		-- ONLY FLEX WEAPONS HERE
		local grip		= nil
	...
	if (second and not second:isBroken() and second:hasTag(FWPGetItemTag("Flex"))) then		-- ONLY FLEX WEAPONS HERE
		local grip		= nil
...
function ReEquipIt(player, weapon)
	...
	if showMag and instanceof(weapon,"HandWeapon") and weapon:isAimedFirearm() then
		showMag(weapon)
	end
...
function MeleeFlexTransform(player)
	...
	if (weapon) and (instanceof(weapon,"HandWeapon")) then
		if	weapon:hasTag(FWPGetItemTag("Flex")) then										-- CHECK IF WEAPON IS FLEX
			...
			ReEquipIt(attacker,weapon)
		elseif weapon:hasTag(FWPGetItemTag("Flip")) then										-- DEPLOY ON SWING LIKE ASP, ETC.
...
function MeleeRestTransform(player)
	...
	if	(weapon) and (instanceof(weapon,"HandWeapon")) and weapon:hasTag(FWPGetItemTag("Flex")) then		-- CHECK IF WEAPON IS FLEX
...
function ControllerMeleeGrip(player, hand)
	...
	if attacker ~= nil then				-- PREVENT RESPAWN ERRORS
		weapon =	attacker:getPrimaryHandItem()
		if		attacker:isRunning() or attacker:isDoShove() or weapon:hasTag(FWPGetItemTag("Flex")) then
				DebugSay(3,"SKIP...")
				return
		end
	end
...
function CycleActionFire(player)
	...
	if	(weapon) and (instanceof(weapon,"HandWeapon")) then
				-- FWP B42 shotgun cycle guard: legacy open/closed sprite re-equip cancels B42 shotgun fire.
		if weapon:isAimedFirearm() and isShotgun(weapon) then
...
function CycleActionEnd(player)
	...
	if (weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) then
			if weapon and instanceof(weapon,"HandWeapon") and weapon:isAimedFirearm() and isShotgun(weapon) then
		attacker:getModData().CycleAction = 0
		return
	end
local Open		= weapon:getModData().SpriteOPEN
...
Events.OnKeyPressed.Add(WeaponNormalFireModeToggle)
	...
	--		DebugSay(2,"Player Bumped")
			...
			for i = 1, items:size()-1 do
				...
				if item ~= nil and (item:getCategory() == "Clothing") and (item:isEquipped() == true) then
					if item:hasTag(FWPGetItemTag("Shield")) then
						shield = item
