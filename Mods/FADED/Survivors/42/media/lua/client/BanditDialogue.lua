...
local function canTalkToSurvivor(brain)
    ...
    return programName == "Looter"
        or programName == "Companion"
        or programName == "CompanionGuard"
        or programName == "SurvivorBase"
        or programName == "RecruitableSurvivor"
end
...
function BanditDialogues.doRandomDialogue(player, zombie, topic)
    ...
    if not canTalkToSurvivor(brain) then
        return
    end
...
------------------------------------------------
--- Get relationship for a friendly survivor, or nil if dialogue is unavailable
------------------------------------------------
            ...
            if relationship.relation >= category.min_relation then
                if category.inside_category == category_uniqueId and not addedCategories[category.unique_id] then
                    if BanditDialogues.categoryHasVisibleOptions(category.unique_id, relationship) then
                        ...
                        friendlyContext = ISContextMenu:getNew(context)
