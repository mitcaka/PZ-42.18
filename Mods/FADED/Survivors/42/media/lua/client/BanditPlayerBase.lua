...
BanditPlayerBase.Regenerate = function(baseId)
    ...
    local function updateRealObjects(baseId, square)
        ...
        for i=0, objects:size()-1 do
            ...
            if object.getWaterAmount then
                waterAmount = object:getWaterAmount() or 0
            end
...
BanditPlayerBase.OnTick = function(numberTicks)
    if isServer() then return end
    if not GetBanditModData then return end

    local gmd = GetBanditModData()
    if not gmd or not gmd.Bases then return end

    local hasBase = false
    for _, _ in pairs(gmd.Bases) do
        hasBase = true
        break
    end
    if not hasBase then return end

    BanditPlayerBase.tickCounter = (BanditPlayerBase.tickCounter or 0) + 1
    BanditPlayerBase.Update(numberTicks or BanditPlayerBase.tickCounter)
end

Events.OnTick.Remove(BanditPlayerBase.OnTick)
Events.OnTick.Add(BanditPlayerBase.OnTick)
